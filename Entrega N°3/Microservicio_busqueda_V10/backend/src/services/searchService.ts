...
export class SearchService {
  ...
  static async getClicksByProduct(productId: string) {
    return await Click.find({ productId })
      .sort({ clickedAt: -1 })
      .populate('searchId', 'queryText filters');
  }

  static async getClicksBySearch(searchId: string) {
    return await Click.find({ searchId }).sort({ clickedAt: -1 });
  }
}