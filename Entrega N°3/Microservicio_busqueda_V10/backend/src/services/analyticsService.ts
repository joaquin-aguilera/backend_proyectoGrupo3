...
export class AnalyticsService {
   ...
   */
  static async getTopSearchedProducts(limit: number = 6) {
    try {
      // Agregación para contar cuántas veces aparece cada productId en los resultados
      const topProducts = await Search.aggregate([
      ...
      const productosCompletos = await productsService.getProductos();
      ...
      const topProductsWithInfo = topProducts
        .map(item => {
          const producto = productosCompletos.find(
            p => p.id_producto.toString() === item.productId
          );
